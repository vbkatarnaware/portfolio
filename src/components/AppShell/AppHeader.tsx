...
export default function AppHeader({ content }: AppHeaderProps) {
  ...
  return (
    <div
      className="w-full border-b border-white/5 sticky top-0 z-30 backdrop-blur-xl"
      style={{ background: `linear-gradient(to bottom, ${content.accent}33, rgba(28, 28, 30, 0.85))` }}
    >
      <div className="max-w-4xl mx-auto flex items-center gap-4 px-6 md:px-8 py-5">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-[24px] font-bold tracking-tight text-white leading-tight">{content.name}</h1>
          <p className="text-[12.5px] font-medium text-white/50 truncate">{content.tagline}</p>
        </div>
        {hasActions && (
          <div className="hidden sm:flex items-center gap-2 shrink-0">
            {content.externalUrl && (
              <a
                ...
                className="h-8 w-8 bg-white text-black rounded-full flex items-center justify-center hover:bg-white/90 transition-colors shadow-lg"
              >
